...
- HSV 마커 4개 검출 및 perspective matrix 계산
...
pipeline/
  ...
  marker_detection.py     마커 검출, 마커 config 저장/로드, perspective matrix 계산
...
가상환경을 활성화한 뒤에는 `python3`로 실행합니다.
...
```bash
python3 pipeline/main.py --image test_marker.png --mode hsv
```

K-Means 모드:

```bash
python3 pipeline/main.py --image test_marker.png --mode kmeans --kmeans-k 4 --include-clusters 1,2,3
```

K-Means 추출 후 grouping JSON까지 생성:

```bash
python3 pipeline/main.py \
  --image test_marker.png \
  --mode kmeans \
  --kmeans-k 4 \
...
```bash
python3 pipeline/main.py --image test_marker.png --mode kmeans --kmeans-k 4 --include-clusters 1,2,3 --debug --run-grouping
```

OpenCV 창 표시:

```bash
python3 pipeline/main.py --image test_marker.png --debug --show
```
...
```bash
python3 pipeline/main.py --image test_marker.png --mode kmeans --output-dir ../test_image_output/my_run
```

디버그 폴더 변경:

```bash
python3 pipeline/main.py --image test_marker.png --debug --debug-dir ../test_image_output/my_debug
```

## 마커 Config 재사용

마커 검출 결과와 perspective matrix는 기본적으로 `config/marker_config.json`에 저장됩니다.

파일이 있으면 다음 실행부터 HSV 마커 검출을 다시 하지 않고 저장된 matrix를 재사용합니다.

```bash
python3 pipeline/main.py --image test_marker.png --mode kmeans --marker-config config/marker_config.json
```

마커를 다시 검출하고 config를 덮어쓰기:

```bash
python3 pipeline/main.py --image test_marker.png --mode kmeans --refresh-markers
```

기본 마커 색상은 `red`입니다. 도면에 빨간 동선이나 아이콘이 많으면 마젠타 마커를 사용하는 것이 안전합니다.
...
python3 pipeline/main.py \
  ...
  --refresh-markers \
  --marker-config config/test1_marker_config.json \
  --output-dir ../test_image_output/test1/output \
  --debug \
  --debug-dir ../test_image_output/test1/debug \
  --run-grouping
```
...
처음 보는 이미지에서는 어떤 K-Means cluster가 바닥인지 알 수 없으므로 단계를 나누는 것이 안전합니다.
...
python3 pipeline/main.py \
  ...
  --refresh-markers \
  --marker-config config/test1_marker_config.json \
  --output-dir ../test_image_output/test1/check_clusters \
  --debug \
  --debug-dir ../test_image_output/test1/check_clusters/debug
```

아래 폴더에서 바닥 색상에 해당하는 cluster 번호를 확인합니다.

```text
../test_image_output/test1/check_clusters/debug/clusters/
```

2단계: 선택한 cluster로 최종 polygon + grouping 생성

예를 들어 바닥 cluster가 `1,2,4`라면:
...
```text
../test_image_output/test1/final/floor_polygons.json
../test_image_output/test1/final/polygon_groups.json
../test_image_output/test1/final/debug/polygon_groups.png
```
...
```bash
python3 pipeline/main.py --image test_marker.png --mode hsv --color-range floor_blue
```
...
```bash
python3 pipeline/main.py --image test_marker.png --mode kmeans --open-kernel 3 --close-kernel 5
python3 pipeline/main.py --image test_marker.png --mode kmeans --open-kernel 0 --close-kernel 7
```
...
```bash
python3 tests/test_replay_color_clusters.py --image test_marker.png
```

다른 cluster 선택:

```bash
python3 tests/test_replay_color_clusters.py --image test_marker.png --include-clusters 1,2
```

다른 morphology 값 시험:

```bash
python3 tests/test_replay_color_clusters.py --image test_marker.png --open-kernel 0 --close-kernel 7
```

원본 좌표 기준 cluster별 폴리곤 preview 저장:

```bash
python3 tests/test_replay_color_clusters.py --image test_marker.png --save-cluster-polygons
```

정사영 + auto-centering 후 cluster 색으로 채운 이미지 저장:

```bash
python3 tests/test_replay_color_clusters.py --image test_marker.png --save-centered-color-polygons
```
...
```bash
python3 tests/test_epsilon_sweep.py --image test_marker.png
```

다른 값 비교:

```bash
python3 tests/test_epsilon_sweep.py --image test_marker.png --ratios 0.002,0.004,0.006
```
...
```bash
python3 pipeline/polygon_grouping.py \
...
```bash
python3 pipeline/polygon_grouping.py --adjacency-mode distance
```
...
```bash
python3 pipeline/polygon_grouping.py --target-groups 2
```

또는 층 후보 개수라는 의미로 다음 alias를 사용할 수 있습니다.

```bash
python3 pipeline/polygon_grouping.py --target-layers 2
```
...
```bash
python3 tests/test_polygon_grouping.py
```
...
```bash
python3 tests/test_polygon_grouping.py --no-validate
```
